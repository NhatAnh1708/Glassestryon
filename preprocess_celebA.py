...
part_dict = {}
with open("{0}/Eval/list_eval_partition.txt".format(celebA_path)) as file:
	for line in file:
		token = line.strip().split()
		part_dict[token[0]] = token[1]

attr_dict = {}
with open("{0}/Anno/list_attr_celeba.txt".format(celebA_path)) as file:
	skip2rows = 0
	for line in file:
		skip2rows += 1
...
for type in ["train","test"]:
	...
	for i in range(len(L)):
		key = L[i]
		#img = imageio.imread(f"{celebA_path}/img_align_celeba_png/{key[:-4]}.png")
		img = imageio.imread("{0}/img_align_celeba/{1}".format(celebA_path,key[:-4]+".jpg"))
		images[i] = img
